...
#
# The generated package contains headers, release/debug libraries, and the
# complete PROJ data directory (including proj.db). No system installation or
...
# - 7-Zip (7za on Linux, 7z on macOS, or 7z.exe on Windows)
#
# Usage:
# - Run this script from an empty temporary directory using Git Bash.
# - Copy or publish the resulting proj_<platform>.7z archive.
# - Extract it beneath externals/proj in the esmini source tree.
#
...
if [[ "$OSTYPE" == "msys" || "$OSTYPE" == "cygwin" ]]; then
    ...
    TARGET_DIR="v10"
    ARCHIVE_NAME="proj_windows.7z"
    ARCHIVE_TOOL="${PROGRAMFILES}/7-Zip/7z.exe"
    LIB_EXTENSION="lib"
    LIB_PREFIX=""
elif [[ "$OSTYPE" == linux-gnu* ]]; then
    GENERATOR_ARGUMENTS=(-G "Unix Makefiles")
    TARGET_DIR="linux"
    ARCHIVE_NAME="proj_linux.7z"
    ARCHIVE_TOOL="7za"
    LIB_EXTENSION="a"
    LIB_PREFIX="lib"
...
if ! command -v "$ARCHIVE_TOOL" >/dev/null 2>&1 && [[ ! -x "$ARCHIVE_TOOL" ]]; then
    echo "7-Zip executable '$ARCHIVE_TOOL' was not found." >&2
    exit 1
fi
...
target_link_libraries(sqlite3 PRIVATE sqlite3_static)
...
build_proj() {
    ...
    local common_args=(
        ...
        -DSQLite3_LIBRARY_RELEASE="$sqlite_release"
        -DSQLite3_LIBRARY_DEBUG="$sqlite_debug"
...
package_proj() {
    ...
    local package_dir="$ROOT_DIR/$TARGET_DIR"
    ...
    rm -rf "$package_dir"
    mkdir -p "$package_dir/include" "$package_dir/lib" "$package_dir/share"
    cp -R "$INSTALL_DIR/include/." "$package_dir/include/"
    cp "$release_library" "$package_dir/lib/"
    cp "$debug_library" "$package_dir/lib/${LIB_PREFIX}projd.${LIB_EXTENSION}"
    cp "$INSTALL_DIR/lib/${LIB_PREFIX}sqlite3.${LIB_EXTENSION}" "$package_dir/lib/"
    cp "$INSTALL_DIR/lib/${LIB_PREFIX}sqlite3d.${LIB_EXTENSION}" "$package_dir/lib/"
    cp -R "$INSTALL_DIR/share/proj" "$package_dir/share/"

    "$ARCHIVE_TOOL" a -r "$ARCHIVE_NAME" -m0=LZMA -bb1 -spf -snl "$TARGET_DIR"/*
}
...
echo "Created $ARCHIVE_NAME. Extract it under externals/proj/."